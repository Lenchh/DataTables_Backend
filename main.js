function DataTable(config, data) {
  ...
  if (typeof data === "undefined") {
    getUsers(config.apiUrl).then(users => { table.appendChild(createRow(users, config.columns, config.apiUrl, config)); console.log(users[10])});
  } else {
...
function addModalWindow(config) {
  const modalWindow = document.createElement("div");
  modalWindow.classList.add("modal-window");
  const inputContainer = document.createElement("div");
  inputContainer.classList.add("input-container");
  for (const column of config.columns) {
    const currentInput = createInput(column);
    if(currentInput)
  ...
  saveButton.onclick = function() {saveData(modalWindow, config)};
...
async function saveData(modalWindow, config) {
  let newValue = {};
